    ...
    <div className="overflow-hidden rounded-[22px] border border-line bg-white shadow-card">
      <div className="overflow-x-auto">
        <table className="w-full min-w-[620px] border-collapse text-left">
          <thead className="bg-mist text-xs uppercase tracking-[0.12em] text-slate">
            <tr>
          ...
          <tbody className="divide-y divide-line">
            {projects.map((project) => (
              <tr key={project.name} className="transition hover:bg-[#FAFBFF]">
                ...
                <td className="px-6 py-5 text-sm text-slate">{project.frontend}</td>
                <td className="px-6 py-5 text-sm text-slate">{project.backend}</td>
                <td className="px-6 py-5 text-sm text-slate">{project.database}</td>
              </tr>
            ))}
          </tbody>
        </table>
